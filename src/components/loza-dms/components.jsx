...
// ─── UI Primitives ────────────────────────────────────────────
export const inputCls = "w-full bg-transparent border-b border-stone-200 hover:border-stone-300 focus:border-blue-500 outline-none py-1 text-sm text-stone-700 focus:text-stone-900 placeholder:text-stone-300 transition-all";
  ...
  return (
    <div className="bg-stone-50 border border-stone-200 rounded-2xl p-5 mb-5 flex items-center gap-5">
      ...
      <div className="flex-1 min-w-0">
        ...
        <p className="text-[10px] text-blue-600 uppercase tracking-[0.25em] font-semibold mt-1">{COMPANY.division}</p>
        <div className="flex flex-wrap gap-x-4 gap-y-0.5 mt-1.5 text-xs text-stone-500">
          ...
          <a href={`mailto:${COMPANY.email}`} className="text-blue-500 hover:text-blue-700 transition-colors">{COMPANY.email}</a>
        </div>
      </div>
    </div>
  );
    ...
    <div className="bg-stone-50 border border-stone-200 rounded-2xl p-5 mb-5">
      <div className="flex items-start justify-between gap-6">
        <div className="flex items-center gap-3">
          <span className="inline-block bg-blue-600 text-white text-xs font-bold uppercase tracking-widest px-3 py-1 rounded-full">{label}</span>
        ...
        <div className="grid grid-cols-[auto_1fr] gap-x-4 gap-y-2 items-center text-right">
          ...
          <input className="bg-white border border-stone-200 focus:border-blue-400 rounded-lg px-2 py-1 text-stone-800 text-xs outline-none w-32 ml-auto text-right" placeholder={`${prefix}-XXX`} value={doc.number} onChange={e => update("number", e.target.value)} />
          <span className="text-stone-500 text-xs uppercase tracking-wider text-left">Date</span>
          <input type="date" className="bg-white border border-stone-200 focus:border-blue-400 rounded-lg px-2 py-1 text-stone-800 text-xs outline-none w-32 ml-auto" value={doc.date} onChange={e => update("date", e.target.value)} />
          <span className="text-stone-500 text-xs uppercase tracking-wider text-left">Supervisor</span>
          <input className="bg-white border border-stone-200 focus:border-blue-400 rounded-lg px-2 py-1 text-stone-800 text-xs outline-none w-32 ml-auto text-right uppercase" value={doc.supervisor} onChange={e => update("supervisor", e.target.value)} />
        </div>
      </div>
    </div>
  );
    ...
    <div className="mb-5">
      ...
      <div className="border border-stone-200 rounded-xl overflow-hidden">
        <table className="w-full text-sm">
          ...
          <tbody>
            {doc.lineItems.map((item, idx) => (
              <tr key={item.id} className="border-b border-stone-100 hover:bg-blue-50/30 transition-colors group">
                ...
                {isInvoice && <td className="py-2 px-3 text-center"><input type="checkbox" className="accent-blue-500 w-4 h-4 cursor-pointer" checked={item.invoiced ?? false} onChange={e => updateLineItem(item.id, "invoiced", e.target.checked)} /></td>}
        ...
        <div className="px-3 py-2 bg-stone-50 border-t border-stone-200">
          <button onClick={addLineItem} className="text-xs text-blue-600 hover:text-blue-800 font-medium flex items-center gap-1.5 transition-colors">
...
export function TotalsPanel({ doc, update }) {
  ...
  function Row({ label, value, editable, onEdit, highlight, bold }) {
    return (
      <div className={`flex justify-between items-center py-2 px-3 rounded-lg ${highlight ? "bg-blue-50 border border-blue-200" : ""}`}>
        <span className={`text-xs uppercase tracking-wider ${bold ? "font-bold text-stone-800" : "text-stone-500"}`}>{label}</span>
        {editable ? (
          <input type="number" className={`bg-transparent outline-none text-right w-28 border-b border-stone-200 hover:border-stone-300 focus:border-blue-500 text-sm ${bold ? "font-bold text-stone-900" : "text-stone-700"}`} value={value || ""} onChange={e => onEdit?.(parseFloat(e.target.value) || 0)} placeholder="0" />
        ) : (
          <span className={`text-sm ${highlight ? "text-blue-700 font-bold" : bold ? "font-bold text-stone-900" : "text-stone-700"}`}>{formatCurrency(value)}</span>
        )}
      </div>
    );
  }
...
// ─── Proposal Footer ──────────────────────────────────────────
export function ProposalFooter({ doc }) {
    ...
    <div>
      <div className="bg-stone-50 border border-stone-200 rounded-2xl p-5 mb-5">
        <div className="flex items-center justify-between">
          <span className="inline-block bg-blue-600 text-white text-xs font-bold uppercase tracking-widest px-3 py-1 rounded-full">Terms & Conditions</span>
            ...
            <ul className="p-4 space-y-2">
              {section.items.map((item, ii) => (
                <li key={ii} className={`flex gap-2.5 text-sm text-stone-700 ${item.startsWith("o ") || item.startsWith("This includes") ? "ml-5 text-stone-500" : ""}`}>
                  <span className="text-blue-400 shrink-0 mt-0.5">{item.startsWith("o ") || item.startsWith("This includes") ? "◦" : "•"}</span>
      ...
      <div className="mt-6 border border-stone-200 rounded-xl p-5 bg-stone-50">
        <h3 className="text-xs font-bold uppercase tracking-wider text-blue-600 mb-4">Signature Block</h3>
        <div className="grid grid-cols-2 gap-8">
          ...
          <div className="space-y-4">
            ...
            <div><p className="text-sm font-medium text-stone-800 border-b border-stone-300 pb-1 mb-1">{COMPANY.contact}</p><p className="text-[10px] uppercase tracking-wider text-stone-400">Printed Name</p></div>
...
// ─── Document Form ────────────────────────────────────────────
export function DocumentForm({ type }) {
  const { doc, update, updateClient, updateLineItem, addLineItem, removeLineItem, reset } = useDocumentState(type);
    ...
    <div>
      <div className="flex items-center justify-between mb-5 sticky top-0 z-10 bg-white/95 backdrop-blur-sm py-3 border-b border-stone-100 -mx-6 px-6 no-print">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs font-bold text-blue-700 uppercase tracking-wider">{DOC_LABELS[type]}</span>
        </div>
        <div className="flex gap-2">
          ...
          <button onClick={() => window.print()} className="text-xs text-white bg-blue-600 hover:bg-blue-700 transition-colors px-4 py-1.5 rounded-lg font-medium flex items-center gap-2">
